...
However, in this particular case, the (Nx x Nx) matrix we would get is diagonal,
as the linear operator only consists of ((Dx[j]*Dx[j] + Dy[i]*Dy[i])**2).
...
for i in range(0, Nyc):
    for j in range(0, Nxc):
        L[i, j]     = kappa*((Dx[j]*Dx[j] + Dy[i]*Dy[i])**2);
