...
###########################################
'''
Code to integrate 2d Swift-Hohenberg equation (u_t = -(1+\nabla^{2})^{2}u - \mu u + \nu u^{2} - u^{3}) with periodic boundaries using Fourier spectral method.
...
# N0 = NL operator in Fourier space fft2(nu*u^2 - u^3)
...
as the linear operator only consists of ((1.0 + Dx[j]*Dx[j] + Dy[i]*Dy[i])**2) - mu;
...
for i in range(0, Nyc):
    for j in range(0, Nxc):
        L[i, j]     = -((1.0 + Dx[j]*Dx[j] + Dy[i]*Dy[i])**2) - mu;
        Ainv[i, j]  = (1.0 - 0.5*dt*L[i, j])**(-1)
        B[i, j]     = (1.0 + 0.5*dt*L[i, j])



N0 = fft2(nu*multiply(u0, u0) - multiply(u0, multiply(u0, u0)));
N1 = N0;
...
for n in range (0, Nt+1):
    ...
    N0 = fft2(nu*multiply(u0, u0) - multiply(u0, multiply(u0, u0)));
    N1 = fft2(nu*multiply(u1, u1) - multiply(u1, multiply(u1, u1)));

    v2 = multiply(Ainv, multiply(B, v1)) + 1.5*dt*N1 - 0.5*dt*N0
