...
###########################################
'''
Code to integrate 1d heat equation (u_t = nu*u_xx) with periodic boundaries using Fourier spectral method. Crank-Nicholson CN was used. 
- Pratik Aghor
'''
###########################################
...
nsave = 5;
Nt = 200;
nu = 0.5; #diffusion constant of u_t = nu*u_xx
...
Nx = 256; dt = 0.1; x = (1.0*Lx/Nx)*arange(0, Nx);
#print "x=", x
...
for n in range (1, Nt):
    ...
    if n % nsave == 0:
        plt.savefig("heat_images/heatCN_"+str(n/nsave)+".png")
