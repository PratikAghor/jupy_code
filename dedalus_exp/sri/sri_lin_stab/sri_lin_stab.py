...
B00[0, 0:Nr+1] = 0.0; B00[Nr, 0:Nr+1] = 0.0;
B11[0, 0:Nr+1] = 0.0; B11[Nr, 0:Nr+1] = 0.0;
B22[0, 0:Nr+1] = 0.0; B22[Nr, 0:Nr+1] = 0.0;
...
for k_counter in range(0, Nk):
    ...
    A01 = np.multiply( (2.0*Omega - (eta/(1.0-eta))*(1.0/Re)*2.0*1j*m*np.multiply(oneByr, oneByr)), D0r)
    A04 = - D1_g2gl_c

    A10 = np.multiply((- Z + (eta/(1.0-eta))*(1.0/Re)*2.0*1j*m*np.multiply(oneByr, oneByr)), D0r)
